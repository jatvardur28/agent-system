...
"""
  ...
  # Агент №3 - hyper 2: Исследователь 1 (Использует OpenRouter LLM, чтобы иметь доступ к web_search)
database.add_or_update_agent(
    "agent3_hyper2", "Hyper_2 (Исследователь 1)", "openrouter", "openrouter/mistralai/mistral-7b-instruct", # <-- ИЗМЕНЕНО: теперь OpenRouter
...
# Агент №4 - hyper 3: Исследователь 2 (Использует OpenRouter LLM, чтобы иметь доступ к web_search)
database.add_or_update_agent(
    "agent4_hyper3", "Hyper_3 (Исследователь 2)", "openrouter", "openrouter/mistralai/mistral-7b-instruct", # <-- ИЗМЕНЕНО: теперь OpenRouter
    """Вы - специализированный агент-исследователь. Ваша основная задача - получить инструкцию и запрос, провести тщательный интернет-поиск, используя **доступный инструмент `web_search`**. Соберите наиболее релевантную информацию. После сбора данных, синтезируйте найденные факты в четкий, структурированный отчет. Убедитесь, что ваш отчет отвечает на поставленную задачу. Если необходимо, используйте инструмент `web_search` несколько раз для уточнения или расширения информации. Предоставьте только полученные данные и выводы, без лишних вступлений."""
)

# Агент №5 - openrouter 0: Поисковый движок (его LLM будет использоваться Агентами 3 и 4 для выполнения web_search)
# Этот агент не будет отдельным исполняемым AgentExecutor в оркестрации.
# Его LLM (ChatOpenAI с привязанными инструментами) будет использоваться Агентами №3 и №4
# как их "движок" для инструмента web_search.
database.add_or_update_agent(
    "agent5_openrouter0", "OpenRouter_0 (Поисковый движок LLM)", "openrouter", "openrouter/mistralai/mistral-7b-instruct",
